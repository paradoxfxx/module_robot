#include <robot_controller/impedance_controller.h>
...
namespace controller
{
  ...
  void ImpedanceController::Update_e(const KDL::JntArray _jnts) {
    ...
    // This format corresponds to the Jacobian. Alternative that needs some work: use quaternion representation 
    //for the orientation part. Then the jacobian needs to be rebuilt.
    for (int i = 0; i < 3; i++) {
    ...
    // careful: only damping on velocity ... // x_dot = J*q_dot // is it right ??
